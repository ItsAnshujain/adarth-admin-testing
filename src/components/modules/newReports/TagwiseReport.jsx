  ...
  const tableDataRevenue = useMemo(() => {
    ...
    const tableRows3 = selectedTags.map(tag => {
      ...
      timeUnits.forEach(timeUnit => {
        const revenueForDate = transformedRevenueData[timeUnit]?.[tag] ?? null;

        if (revenueForDate !== null) {
          row[timeUnit] = (revenueForDate / 100000).toFixed(2);
          totalForTag += revenueForDate;
        } else {
          row[timeUnit] = '-';
        }
      });

      row['Grand Total'] = (totalForTag / 100000).toFixed(2);
      return row;
    });
